...
const SELECTORS = {
  // キーワード入力フィールド
  keywordInput: 'input[data-testid="text-input-field"]',
  // キーワード入力フィールド（代替）
  keywordInputAlt: 'input[placeholder*="キーワード"], input[placeholder*="keyword"]',
  // 追加ボタン
  addButton: 'button[data-testid="confirmationSheetConfirm"]',
  // 追加ボタン（代替）
...
const findAddButton = (): HTMLButtonElement | null => {
  ...
  for (const selector of selectors) {
    ...
    for (const button of buttons) {
      ...
      if ((text.includes('追加') || text.includes('add') || text.includes('確認')) && 
          button.offsetParent && !button.disabled) {
        return button
      }
    }
  }
